...
def call(container,command,cwd='',paths={},remove=True,cpu_set='',sig_default=True):
  ...
  try:
    ...
    if cpu_set:
      ...
      container_obj = client.containers.run(container,command,user=user,volumes=volumes,working_dir=cwd,remove=remove,detach=True,cpu_period=100000,cpu_quota=cpu_set,labels={"prog":"spriggan"})
    else:
      container_obj = client.containers.run(container,command,user=user,volumes=volumes,working_dir=cwd,remove=remove,detach=True,labels={"prog":"spriggan"})
  except:
